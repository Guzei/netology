...
/* ПРОЧТИ МЕНЯ */
/* Игорь, приветствую! Из-за некоторых ограничений гитхаба я не могу в удобном виде оставлять комментарии к коду в интерфейсе гитхаба,
** поэтому я буду писать свои комментарии в многострочных комментариях прям в этом файле и отправлю вам их как PR,
** чтобы вам было удобнее смотреть 
*/
...
/* Начнем сразу с ошибок. Когда я вижу данный enum, я не совсем понимаю смысл отдельных кейсов. 
** Что значит ошибка food ? а что значит ошибка count? если я встречу такую в коде мне по названию будет непонятно для чего они.
** Необходимо внести больше специфики в название: например badFood вместо food, insufficientCount вместо просто count или notEnoughIngridients вместо sorry
** пока что имена ошибок слишком общие 
*/
enum Errors: Error {
...
/* для учебного проекта ок, в реальном вас попросят от смайликов избавиться =) 
** и описать все словами
*/
enum NameOfDish {
...
struct FriedEggs: Dish {
    ...
    let price: UInt = 500
    /* зачем здесь явное указание internal ? */
...
/* если это один заказ, то лучше было не употреблять множественное число в названии, сбивает с толку */
protocol Orders {                                   // MARK: 1.4. Заказы
    ...
    /* зачем здесь set? разве есть смысл в изменении количества блюд в заказе после его создания? */
...
/* тогда реализацию протокола можно было бы назвать 
** ConcreteOrder или OrderImpl */

/* по поводу выбора class/structure
** тут зависит не только от того, как вы будете передавать экземпляры и будете ли наследовать
** а также от того как много и часто вы будете создавать экземпляры,
** создание экземпляра класса в общем случае более трудоемкая задача,
** поэтому если ваши сущности являются просто удобным способом хранения данных и несложной их обработки,
** то я бы выбрал вариант structure (если вы не планируете наследование).
** особенно это справедливо для т.н. моделей - то есть логических сущностей, которые представляют собой структуры данных, которыми вы манипулируете.
**
*/
...
enum Sex { case male, female } /* хорошо их только два =) */
...
final class Garcon: MemberOfStaff {
    ...
    var name: String  /* var? почему не let */
    let sex: Sex
    var age: UInt8 /* var? почему не let */
    ...
    func pushOrder() throws {                       // ! добавляет в начало хранилища с заказами переданный заказ.
        ...
        var dishs: Array<Dish> = []
        /* я правильно понимаю, что если нет хоть одного ингридиента, то весь заказ отменяется? 
        ** может лучше пропускать такие блюда и в результате функции возвращать например блюда, которые не попали в заказ? 
        ** это просто как предложение 
        */
        try preOrder.forEach({ dishName in
            
            /* я бы сделал extension над DishName, которое превращает имя в блюдо 
            ** и вынес бы этот код отсюда 
            */
        ...
        /* корректно, но можно еще использовать removeAll */
        preOrder = []                               // надеюсь так корректно удалять
    }
}
/* лучше сделать также классом, возможно вы захотите сделать потом наследование? 
** например создать ChiefManager
*/
...
final class Butler: Restaurant, CheckFoodsInRoom, PushOrder, PullOrder {
    ...
    /* должен быть массив протоколов: 
    ** всегда надо стремиться зависеть от абстракций, а не от конкретных реализаций 
    */
    /* я бы выбрал здесь очередь вместо массива,
    ** например взял бы из пакета swiftcollections https://github.com/apple/swift-collections/blob/main/Documentation/Deque.md
    ** потому что вставка элементов не в конец массива неэффективна
    */
    ...
    func initFoodRoom() -> FoodList {
        /* создание локального хранилища лучше сделать после guard, так логичней */
        var foodRoom: FoodList = [:] 
        guard let foodRoomDB = db.dictionary(forKey: name) as? [String:UInt] else {
            print("no data for key \"\(name)\"")
            return foodRoom /* а тут просто вернуть [:] */
        }
        /* можно использовать метод map, он и нужен для таких случаев */
    ...
    func checkFoodsInRoom(foodList: FoodList) throws {
        ...
        /* лучше объединить эти два цикла в один:
        ** то есть сразу проверять и отнимать,
        ** чтобы не гонять циклы два раза
        */
